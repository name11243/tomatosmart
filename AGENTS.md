...
User decision (2026-09-16，一键改 IP): 用户要求制作双击运行的改 IP 脚本，并确认范围“和刚才一样”：仅同步电脑端 MQTT 与摄像头配置，自动备份并应用到后端；不切换网络，不修改网卡地址或 ESP32 保存的配置。保留原摄像头端口与路径、MQTT 凭据和主题，以及 EMQX 节点名、匿名连接和授权配置。
