...
<template>
 <div class="overlay modal-overlay"><section class="modal camera-modal" role="dialog" aria-modal="true" aria-labelledby="camera-heading">
  ...
  <template v-if="mode==='remote'&&!captured">
   ...
   <label for="remote-camera-url">摄像头请求地址</label>
   ...
   <div class="camera-connect"><button :disabled="probing||busy||!remoteUrl.trim()" @click="diagnose">{{probing?'检测中…':'检测连接'}}</button><span class="camera-help">检测会由 Docker 后端依次请求该地址与 /snapshot、/image、/download，并说明每个地址的结果。</span></div>
