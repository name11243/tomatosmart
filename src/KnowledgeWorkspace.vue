...
async function loadOverview(){graphBusy.value=true;try{overview.value=await api('/knowledge/graph?q='+encodeURIComponent(graphFilter.value));}catch(e){overview.value=null;throw e}finally{graphBusy.value=false}}
...
async function saveKnowledge(){if(editorBusy.value)return;editorBusy.value=true;try{await attempt(async()=>{
 ...
 editor.value=null;await load();await statuses();emit('notify','资料已写入 Neo4j，标记为待核验；已有问答快照保持原样。');
...
<template>
 <div class="knowledge-workspace">
  ...
  <div class="knowledge-status" aria-live="polite"><span :class="{unavailable:!db?.connected}"><i class="ri-database-2-line"/> {{db?.connected?'Neo4j 已连接 · '+db.sources+' 条资料':'Neo4j 未连接'}}</span><span :class="{unavailable:!ai?.connected}"><i class="ri-cpu-line"/> {{ai?.connected?'Ollama 已连接':'Ollama 未连接'}}</span><button class="text-button" :disabled="statusBusy" @click="statuses">{{statusBusy?'检查中…':'刷新连接状态'}}</button></div>
  <template v-if="tab==='问答'">
   <section class="knowledge-heading"><h1>从一个问题，开始一次探究</h1><p>本地 AI 结合知识来源整理回答，图谱中的每条关系均来自 Neo4j。</p></section>
   ...
   <div class="query-options"><label>回答方式 <select v-model="mode" :disabled="busy"><option value="local_ai">本地 AI + 知识来源</option><option value="neo4j_graph">仅查询知识资料</option></select></label><label v-if="mode==='local_ai'">本地模型 <select v-model="model" :disabled="busy||!ai?.models?.length"><option v-for="m in ai?.models" :key="m" :value="m">{{m}}</option><option v-if="!ai?.models?.length" :value="model">暂无可用模型</option></select></label><small v-if="busy" role="status">正在检索并整理回答，首次加载本地模型可能需要较长时间。</small><small v-else-if="mode==='local_ai'&&!ai?.connected">{{ai?.message}} 可选择仅查询知识资料。</small></div>
   <div class="knowledge-grid">
    <section class="panel graph-panel"><div class="panel-heading"><div><h2>知识图谱</h2><p>{{answer?'本次问题的来源关系':'全库浏览 · 最多展示 20 条来源，可筛选缩小范围'}}</p></div><button v-if="answer" class="text-button" @click="answer=null;attempt(loadOverview)">浏览全库</button></div>
     ...
     <div class="legend"><span v-for="(label,i) in ['作物','环境','问题','原因','措施','来源']" :key="label"><b :class="'dot dot-'+i"/>{{label}}</span></div>
     <Graph v-if="graphData?.nodes?.length" :data="graphData" @select="selectNode" @source="viewSource"/>
     <div v-else class="empty graph-empty"><i class="ri-node-tree"/><p>{{busy?'正在检索知识关系…':db?.connected?(answer?'没有与此问题匹配的关系。':'暂无匹配资料。新增实际来源后即可建立图谱。'):'图数据库不可用，恢复连接后重试。'}}</p><button v-if="canTeach&&!answer" @click="startEdit()">新增知识资料</button></div>
  ...
  <div v-if="editor" class="modal-backdrop"><form class="modal knowledge-dialog" role="dialog" aria-modal="true" aria-label="编辑知识资料" @submit.prevent="saveKnowledge"><h2>{{editor.id?'编辑知识资料':'新增知识资料'}}</h2><p>仅填写有实际依据的内容；新建或编辑均标记为待核验，历史引用保持原样。</p><div class="knowledge-fields"><label v-for="f in [['title','标题',200],['crop','作物',100],['problem','问题 / 主题',300],['cause','原因 / 关系说明',500],['measure','措施 / 核对方法',1000],['source','来源说明或链接',1000]]" :key="f[0]">{{f[1]}}<input v-model="editor[f[0]]" required :maxlength="f[2]"/></label><label>关键词（顿号分隔）<input v-model="editor.tagsText" maxlength="500"/></label><label>原文明示的环境因素（可留空）<input v-model="editor.envText" maxlength="300" placeholder="如 EC、液位；未提及则留空"/></label></div><label>资料正文<textarea v-model="editor.content" required maxlength="20000" rows="8"/></label><div class="actions"><button type="button" :disabled="editorBusy" @click="editor=null">取消</button><button class="primary" :disabled="editorBusy">{{editorBusy?'保存中…':'保存到 Neo4j'}}</button></div></form></div>
 </div>
</template>

<style scoped>
.modal-backdrop{position:fixed;inset:0;z-index:60;background:#14352666;display:flex;align-items:center;justify-content:center}.graph-panel{display:flex;flex-direction:column;height:650px;min-height:0}.graph-panel .panel-heading{height:auto;flex-shrink:0}.graph-panel :deep(.graph-wrap){height:auto;flex:1;min-height:300px}.graph-panel .relation{position:static;max-width:none;font-size:11px;padding:12px 20px;white-space:normal;border-top:1px solid var(--line)}.answer-panel{height:auto;min-height:0}.query-options select{width:auto}.knowledge-grid{align-items:start}.knowledge-status button{font-size:12px}.graph-panel .legend{padding-bottom:10px}
