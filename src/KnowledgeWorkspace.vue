...
async function saveKnowledge(){if(editorBusy.value)return;editorBusy.value=true;try{await attempt(async()=>{
 ...
 editor.value=null;await load();await statuses();emit('notify','资料已写入 Neo4j 图谱，标记为待核验；已有问答快照保持原样。');
...
<template>
 <div class="knowledge-workspace">
  ...
  <div class="knowledge-status" aria-live="polite"><span :class="{unavailable:!db?.connected}"><i class="ri-node-tree"/> {{db?.connected?'Neo4j 已连接 · '+db.sources+' 条资料':'Neo4j 不可用'}}</span><span :class="{unavailable:!ai?.connected}"><i class="ri-cpu-line"/> {{ai?.connected?'Ollama 已连接':'Ollama 未连接'}}</span><button class="text-button" :disabled="statusBusy" @click="statuses">{{statusBusy?'检查中…':'刷新状态'}}</button></div>
  <template v-if="tab==='问答'">
   <section class="knowledge-heading"><h1>从一个问题，开始一次探究</h1><p>本地 AI 结合知识来源整理回答，关系由 Neo4j 实时检索。</p></section>
   ...
   <div class="knowledge-grid">
    <section class="panel graph-panel"><div class="panel-heading"><div><span class="eyebrow">KNOWLEDGE OCEAN</span><h2>知识图谱观察窗</h2><p>{{answer?'本次问题的来源关系 · 节点仍可自由拖动':'完整知识海面 · 移动观察窗探索全部关系，也可筛选聚焦'}}</p></div><button v-if="answer" class="text-button" @click="answer=null;attempt(loadOverview)">返回完整图谱</button></div>
     ...
     <div v-else class="empty graph-empty"><i class="ri-node-tree"/><p>{{busy?'正在检索知识关系…':db?.connected?(answer?'没有与此问题匹配的关系。':'暂无匹配资料。新增实际来源后即可建立图谱。'):'Neo4j 不可用，请检查图数据库连接。'}}</p><button v-if="canTeach&&!answer" @click="startEdit()">新增知识资料</button></div>
