...
前端图谱改为 Bolt/Cypher 查询返回的真实节点与关系；页面标识「Neo4j 图数据库 · 实时查询」。截图 docs/qa/neo4j-graph.png。14 项测试通过，新增知识跨驱动持久化读回，直接修改数据库原因和关系后问答与图谱同步变化；停库返回 503，重启保留 19 个节点、18 条关系。启动脚本重复执行不覆盖 Neo4j 已有记录，迁移输出 imported=[]。数据库内容仍是明确标记的 4 条待核验示例知识。
