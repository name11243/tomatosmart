...
def test_real_mqtt_roundtrip(protocol):
 ...
 if protocol=='tomato_v1_1':
  config.update(protocol=protocol,qos=0,device_id='HY-003',telemetry_topic='tomato_hnsw0001/telemetry',command_topic='tomato_hnsw0001/set',ack_topic='tomato_hnsw0001/result')
 service.save_config(config)
 endpoint=mqtt.Client(mqtt.CallbackAPIVersion.VERSION2,client_id='test-device');received=[];wire_options=[];ack_now=threading.Event()
 def on_message(c,u,m):
  payload=json.loads(m.payload);received.append(payload);wire_options.append((m.qos,m.retain))
 endpoint.on_message=on_message
 try:
  endpoint.connect('127.0.0.1',port);endpoint.subscribe('tomato_hnsw0001/set' if protocol=='tomato_v1_1' else 'hydroponics/HY-003/command',qos=0 if protocol=='tomato_v1_1' else 1);endpoint.loop_start()
  service.start();assert service.connected and service.subscribed
  ...
  if protocol=='tomato_v1_1':
   ...
   endpoint.publish('tomato_hnsw0001/state',json.dumps(state),qos=0,retain=True).wait_for_publish()
  ...
  if protocol=='tomato_v1_1':
   ...
   assert wire_options[0]==(0,False)
   endpoint.publish('tomato_hnsw0001/result',json.dumps({'request_id':cmd['id'],'cmd':'03','success':True,'applied':True,'data':{'value':1},'message':'pump updated'}),qos=0).wait_for_publish()
  ...
  if protocol=='tomato_v1_1':
   ...
   count=len(s.allof('commands'))
   with pytest.raises(ValueError,match='离线'):service.command('HY-003','pump',False)
   assert len(s.allof('commands'))==count
 finally:
